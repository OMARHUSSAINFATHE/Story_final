...
class _StatusViewPageState extends State<StatusViewPage> {
  ...
  // ✅ مرجع للـ video widget عشان نوقفه لما نضغط
  final GlobalKey<_VideoStoryWidgetState> _videoKey = GlobalKey();
      ...
      stories.map((story) async {
        ...
        try {
          if (_isVideo(story)) {
            ...
            f ??= await _cacheService
                .cacheVideo(story)
                .timeout(const Duration(seconds: 30))
                .catchError((_) => null);
            if (f != null && mounted)
              setState(() => _cachedVideoPaths[story] = f!.path);
          } else {
            ...
            f ??= await _cacheService
                ...
                .catchError((_) => null);
            if (f != null && mounted)
              setState(() => _cachedImagePaths[story] = f!.path);
          }
        } catch (_) {}
      }),
    );
  ...
  void _pauseProgress() {
    ...
    _videoKey.currentState?.pause(); // ✅ وقّف الفيديو لو موجود
  }

  void _resumeProgress() {
    if (!_isPaused) return;
    setState(() => _isPaused = false);
    _videoKey.currentState?.resume(); // ✅ استكمل الفيديو
  }
  ...
  bool _isVideo(String s) => [
    ...
    '.mkv',
    '.webm',
  ].any((e) => s.toLowerCase().endsWith(e));
  ...
  Future<void> _deleteCurrentStory() async {
    ...
    final apiId =
        widget.storyModel.storiesApiIds[story] ?? widget.storyModel.apiId;
    ...
    if (apiId != null) {
      ...
      if (!deleted && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('  حدث خطأأثناءالحذف الحالة '),
            backgroundColor: Colors.red,
          ),
        );
        return;
  ...
  Widget _buildStoryContent(int index) {
    ...
    if (_isVideo(story)) {
      ...
      final isNetworkFallback =
          story.startsWith('http') && !_cachedVideoPaths.containsKey(story);
    ...
    if (_isUrl(story)) {
      ...
      if (cached != null) {
        return SizedBox.expand(
          child: Image.file(File(cached), fit: BoxFit.contain),
        );
      }
      return SizedBox.expand(
        child: Image.network(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_currentUploadTime != null ||
                              widget.storyModel.uploadedAt != null)
                            Text(
                              _formatTime(
                                _currentUploadTime ??
                                    widget.storyModel.uploadedAt!,
                              ),
                              style: const TextStyle(
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: TextField(
                            ...
                            onSubmitted: (v) {
                              if (v.trim().isNotEmpty) _sendReply();
                            },
                          ),
                        ),
                      ),
