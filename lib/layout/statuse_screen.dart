...
class _StatusPageState extends State<StatusPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (friends.isEmpty) {
        ...
        setState(() { _friends = demos; _viewedMap = viewedMap; _isLoading = false; });
      } else {
        setState(() { _friends = friends; _viewedMap = viewedMap; _isLoading = false; });
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _loadFriendsFromCache() async {
    try {
      ...
      return list.cast<Map<String, dynamic>>().where((item) {
        ...
        final remainingHrs = _expiryHours - DateTime.now().difference(savedAt).inHours;
  ...
  Future<void> _saveFriendsToCache(List<Map<String, dynamic>> friends) async {
    try {
      ...
      await CacheService.imageCache.putFile(_friendsKey, bytes, fileExtension: 'json');
    } catch (_) {}
  }
  ...
  Future<Map<String, Set<int>>> _loadViewedFromCache() async {
    try {
      ...
      return data.map((k, v) => MapEntry(k, Set<int>.from((v as List).map((e) => e as int))));
    } catch (_) { return {}; }
  }
  ...
  Future<void> _saveViewedToCache(Map<String, Set<int>> viewedMap) async {
    try {
      ...
      await CacheService.imageCache.putFile(_viewedKey, bytes, fileExtension: 'json');
    } catch (_) {}
  }
  ...
  List<Map<String, dynamic>> get _sortedFriends {
    ...
    for (final item in _friends) {
      ...
      vSet.isNotEmpty && vSet.length >= stories.length
          ? viewed.add(item) : unviewed.add(item);
    }
    final sorted = [...unviewed, ...viewed];
    if (_searchQuery.isEmpty) return sorted;
    return sorted.where((i) =>
        (i['name'] as String).toLowerCase().contains(_searchQuery.toLowerCase())).toList();
  }
  ...
  Future<void> _createTextStatus() async {
    ...
    if (result is StatusModel && mounted) {
      ...
      setState(() {
        ...
        _myStoriesUploadTime = Map.from(_myStoriesUploadTime)..[key] = DateTime.now();
  ...
  Future<void> _createMediaStatus(ImageSource source, String type) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => MediaStatusCreator(source: source, type: type)),
    );
    if (result is StatusModel && mounted) {
      ...
      setState(() {
        ...
        _myStoriesUploadTime = Map.from(_myStoriesUploadTime)..[path] = DateTime.now();
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ListTile(
              ...
              onTap: () { Navigator.pop(context); _createTextStatus(); },
            ),
            ...
            ListTile(
              ...
              onTap: () { Navigator.pop(context); _createMediaStatus(ImageSource.camera, 'IMAGE'); },
            ),
            ListTile(
              leading: const Icon(Icons.photo_library, color: Colors.grey),
              title: const Text('Gallery Image'),
              onTap: () { Navigator.pop(context); _createMediaStatus(ImageSource.gallery, 'IMAGE'); },
            ),
            ListTile(
              leading: const Icon(Icons.videocam, color: Colors.grey),
              title: const Text('Camera Video'),
              onTap: () { Navigator.pop(context); _createMediaStatus(ImageSource.camera, 'VIDEO'); },
            ),
            ListTile(
              leading: const Icon(Icons.video_library, color: Colors.grey),
              title: const Text('Gallery Video'),
              onTap: () { Navigator.pop(context); _createMediaStatus(ImageSource.gallery, 'VIDEO'); },
            ),
          ]),
        ),
      ),
  ...
  void _openFriendStatus(int filteredIndex) {
    ...
    for (int i = 0; i < stories.length; i++) {
      if (!viewedSet.contains(i)) { startIndex = i; break; }
    }
  ...
  StoryModel _buildMyStatusModel() {
    ...
    return StoryModel(
      ...
      isLocalImage: !_myStories.first.startsWith('http') &&
      ...
      storiesApiIds: Map.fromEntries(              // ✅ كل الـ stories
  ...
  List<Map<String, dynamic>> _getDemoFriends() => [
    {
      'id': 'friend_1', 'name': 'medo 1',
      'avatar': 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800',
      'stories': [
    ...
    {
      'id': 'friend_3', 'name': 'Mohamed Ahmed 3',
      'avatar': 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800',
      'stories': [
        'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=800',
        'https://www.w3schools.com/tags/mov_bbb.mp4',
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              PopupMenuItem(value: 'Status Privacy', child: Text('Status Privacy')),
            ],
          ),
        ],
      ),
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
                  ...
                  onTap: hasStories
                      ? () => Navigator.push(context, MaterialPageRoute(
                            builder: (_) => StatusViewPage(
                              storyModel:         _buildMyStatusModel(), // ✅
                              allStories:         null,
                              storiesUploadTimes: Map.from(_myStoriesUploadTime),
              ...
              final allSeen  = vSet.isNotEmpty && vSet.length >= stories.length;
...
class _MyStatusCard extends StatelessWidget {
  ...
  bool get _firstIsText =>
      hasStories && myStories.first.startsWith('text:');

  String get _textContent =>
      myStories.first.replaceFirst('text:', '');
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(fit: StackFit.expand, children: [
          ...
          else if (myStories.first.startsWith('http'))
            Image.network(myStories.first, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(color: Colors.grey))
          else
            Image.file(File(myStories.first), fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(color: Colors.grey)),
